...
# y = ["Prod. {}".format(i) for i in range(10, 60, 10)]
# x = ["Cycle {}".format(i) for i in range(1, 6)]
#
# qrates = np.array(list("AB"))
# norm = matplotlib.colors.BoundaryNorm(np.linspace(-3.5, 3.5, 8), 6)
# # fmt = matplotlib.ticker.FuncFormatter(lambda x, pos: qrates[::-1][norm(x)])
#
# im, _ = heatmap(data, y, x, ax=ax,
#                 cmap=plt.get_cmap("PiYG", 6), norm=norm,
#                 cbar_kw=dict(ticks=np.arange(-3, 4)   ), format=fmt),
#                 cbarlabel="Quality Rating")
#
# annotate_heatmap(im,
#                     valfmt=fmt,
#                     size=9, fontweight="bold", threshold=-1,
